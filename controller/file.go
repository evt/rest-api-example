...
func (ctr *FileController) Create(ctx echo.Context) error {
	...
	createdFile, err := ctr.services.File.CreateFileMeta(ctx.Request().Context(), &file)
	if err != nil {
		switch {
...
func (ctr *FileController) Get(ctx echo.Context) error {
	...
	file, err := ctr.services.File.GetFileMeta(ctx.Request().Context(), fileID)
	if err != nil {
		switch {
		case errors.Cause(err) == types.ErrNotFound:
			return echo.NewHTTPError(http.StatusNotFound, err)
...
func (ctr *FileController) Delete(ctx echo.Context) error {
	...
	err = ctr.services.File.DeleteFileMeta(ctx.Request().Context(), fileID)
	if err != nil {
		switch {
		case errors.Cause(err) == types.ErrNotFound:
